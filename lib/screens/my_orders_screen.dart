...
// ── Barcha buyurtmalar (kassir uchun) ─────────────────────────────────────────

class AllOrdersScreen extends StatefulWidget {
  const AllOrdersScreen({super.key});

  static final instanceKey = GlobalKey<_AllOrdersScreenState>();
  static void refreshIfMounted() => instanceKey.currentState?._silentRefresh();
...
class _AllOrdersScreenState extends State<AllOrdersScreen>
  ...
  Future<void> _fetch() async {
    try {
      final paid = _filter == 'ALL' ? '' : '&paid=$_filter';
      final data = await Api.get(
          'orders?date=$_dateParam$paid&pageSize=200') as Map<String, dynamic>;
      if (!mounted) return;
  ...
  Future<void> _payOrder(_Order order) async {
    ...
    try {
      final data = await Api.get('payments') as List;
      methods = data
          .map((j) => _PayMethod(id: j['id'] as int, name: (j['name'] ?? '').toString()))
          .toList();
    } catch (_) {}
    if (!mounted) return;
    ...
    return Scaffold(
      ...
      body: Column(children: [
            ...
            Expanded(
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _isToday ? AppColors.primaryLight : AppColors.bg,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.calendar_month, size: 16,
                        color: _isToday ? AppColors.primary : AppColors.textMuted),
                    const SizedBox(width: 8),
                    Text(_dateLabel,
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 15,
                            color: _isToday ? AppColors.primary : AppColors.textDark)),
                    const SizedBox(width: 4),
        ...
        Container(
          ...
          padding: const EdgeInsets.fromLTRB(16, 0, 16, 12),
          child: Row(children: [
            _FilterChip(label: 'Barchasi', active: _filter == 'ALL',
                color: AppColors.textDark, onTap: () => _setFilter('ALL')),
            const SizedBox(width: 8),
            _FilterChip(label: 'Ochiq', active: _filter == 'NO',
                color: AppColors.primary, onTap: () => _setFilter('NO')),
            const SizedBox(width: 8),
            _FilterChip(label: 'Yopiq', active: _filter == 'YES',
                color: AppColors.success, onTap: () => _setFilter('YES')),
            const Spacer(),
            if (!_loading)
              Text('${_orders.length} ta',
                  ...
                  : _orders.isEmpty
                      ...
                      : RefreshIndicator(
                          ...
                          child: ListView.builder(
                            ...
                            itemCount: _orders.length,
                            itemBuilder: (_, i) => _AllOrderCard(
                              order: _orders[i],
                              onPay: _orders[i].paid == 'NO'
                                  ? () => _payOrder(_orders[i])
                                  : null,
                              onReceipt: () => _showReceipt(_orders[i]),
                            ),
  ...
  Widget _buildEmpty() => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(_filter == 'YES' ? Icons.check_circle_outline : Icons.receipt_long_outlined,
              size: 56, color: AppColors.border),
          const SizedBox(height: 12),
          Text('$_dateLabel kuni buyurtma yo\'q',
              style: const TextStyle(color: AppColors.textMuted, fontSize: 15)),
        ]),
      );
}

// ── Barcha buyurtmalar kartochkasi ────────────────────────────────────────────
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(
                    child: Text(
                        order.placeName.isNotEmpty ? order.placeName : 'Yetkazib berish',
                        style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 14,
                            color: AppColors.textDark),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: sBg, borderRadius: BorderRadius.circular(10)),
                    child: Text(sLabel,
                        style: TextStyle(
                            fontSize: 11, fontWeight: FontWeight.bold, color: sColor)),
                  ),
                ]),
                const SizedBox(height: 3),
                Row(children: [
                  ...
                  Text(
                      order.userName.isNotEmpty ? order.userName : '—',
                      style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
                  const SizedBox(width: 10),
                  Text('${order.itemsCount} ta taom  •  $timeStr',
                      style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
                ]),
              ]),
            ),
